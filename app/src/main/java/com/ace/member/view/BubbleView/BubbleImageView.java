package com.ace.member.view.BubbleView;
...
public class BubbleImageView extends android.support.v7.widget.AppCompatImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (bubbleDrawable != null) bubbleDrawable.draw(canvas);
	...
	private void setUp(int left, int right, int top, int bottom) {
		...
		if (sourceDrawable != null) mBitmap = getBitmapFromDrawable(sourceDrawable);
	...
	public static Bitmap getBitmapFromDrawable(Context mContext, Drawable drawable, int width, int height, int defaultSize) {
		...
		try {
			...
			if (width > 0 && height > 0) {
				...
				bitmap = Bitmap.createBitmap(dp2px(mContext, defaultSize), dp2px(mContext, defaultSize), Bitmap.Config.ARGB_8888);
			}
