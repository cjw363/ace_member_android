...
public class SideBar extends View {
	...
	public static final int TEXT_SIZE = Utils.dip2px(12);
	// 触摸事件
	...
	protected void onDraw(Canvas canvas) {
		...
		int singleHeight = height / A_Z.length;// 获取每一个字母的高度

		for (int i = 0; i < A_Z.length; i++) {
			paint.setColor(Color.GRAY);  //设置字体颜色
			paint.setTypeface(Typeface.DEFAULT);  //设置字体
			paint.setAntiAlias(true);  //设置抗锯齿
			paint.setTextSize(TEXT_SIZE);  //设置字母字体大小
			// 选中的状态
			if (i == choose) {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		switch (action) {
			...
			default:
				setBackgroundResource(R.drawable.bg_shape_gray_letter);
