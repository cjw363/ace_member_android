...
public class MyGridPasswordView extends LinearLayout implements MyPasswordView {
	...
	public MyGridPasswordView(Context context) {
		super(context);
		initViews(context);
		init(context, null, 0);
	}

	public MyGridPasswordView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs, 0);
	}

	public MyGridPasswordView(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);
		init(context, attrs, defStyleAttr);
	}

	@TargetApi(Build.VERSION_CODES.LOLLIPOP)
	public MyGridPasswordView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
	...
	private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mTextColor = ta.getColorStateList(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvTextColor);
		if (mTextColor == null)
			mTextColor = ColorStateList.valueOf(getResources().getColor(android.R.color.primary_text_light));
		int textSize = ta.getDimensionPixelSize(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvTextSize, -1);
		if (textSize != -1) {
			this.mTextSize = Util.px2sp(context, textSize);
		}

		mLineWidth = (int) ta.getDimension(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvLineWidth, Util
			.dp2px(getContext(), 1));
		mLineColor = ta.getColor(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvLineColor, DEFAULT_LINECOLOR);
		mGridColor = ta.getColor(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvGridColor, DEFAULT_GRIDCOLOR);
		mLineDrawable = ta.getDrawable(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvLineColor);
		if (mLineDrawable == null) mLineDrawable = new ColorDrawable(mLineColor);
		mOuterLineDrawable = generateBackgroundDrawable();

		mPasswordLength = ta.getInt(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvPasswordLength, DEFAULT_PASSWORDLENGTH);
		mPasswordTransformation = ta.getString(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvPasswordTransformation);
		if (TextUtils.isEmpty(mPasswordTransformation))
			mPasswordTransformation = DEFAULT_TRANSFORMATION;


		mPasswordType = ta.getInt(com.jungly.gridpasswordview.R.styleable.gridPasswordView_gpvPasswordType, 0);
	...
	private void inflaterViews(Context context) {
		...
		mInputView = (ImeDelBugFixedEditText) findViewById(com.jungly.gridpasswordview.R.id.inputView);
		...
		while (index < mPasswordLength) {
			...
			dividerView.setBackgroundDrawable(mLineDrawable);//分割线
	...
	private void notifyTextChanged() {
		...
		if (currentPsw.length() == mPasswordLength) mListener.onInputFinish(currentPsw);
	}
