...
public class NumberRollingTextView extends TextView {
	...
	public NumberRollingTextView(Context context, AttributeSet attrs) {
		this(context, attrs, android.R.attr.textViewStyle);
	}

	public NumberRollingTextView(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);

		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.NumberRollingTextView);
		frameNum = ta.getInt(R.styleable.NumberRollingTextView_frameNum, 30);
		textType = ta.getInt(R.styleable.NumberRollingTextView_textType, MONEY_TYPE);
		useCommaFormat = ta.getBoolean(R.styleable.NumberRollingTextView_useCommaFormat, true);
		runWhenChange = ta.getBoolean(R.styleable.NumberRollingTextView_runWhenChange, true);
	}
	...
	public void startMoneyAnim(String moneyStr) {
		...
		try {
			...
			if (finalMoneyNum == 0) {
				...
				NumberRollingTextView.this.setText(moneyStr);
				return;
			}
			nowMoneyNum = 0;
	...
	public void startNumAnim(String numStr) {
		...
		try {
			...
			if (finalNum < frameNum) {
				...
				NumberRollingTextView.this.setText(numStr);
				return;
			}
			// 默认从0开始动画
			nowNum = 0;
			...
			switch (msg.what) {
				case MONEY_TYPE://金额数字的滚动
					...
					if (useCommaFormat) {
						...
						NumberRollingTextView.this.setText(formatStr);
					} else {
						NumberRollingTextView.this.setText(str);
					}
					...
					if (nowMoneyNum < finalMoneyNum) {
						...
						//已经达到目标的金额数字，显示最终的数字
						if (useCommaFormat) {
							NumberRollingTextView.this.setText(StringUtil.addComma(formatter.format(finalMoneyNum), true));
						} else {
							NumberRollingTextView.this.setText(formatter.format(finalMoneyNum));
						}
					}
					break;
				case NUM_TYPE://普通数字滚动
					...
					if (useCommaFormat) {
						...
						NumberRollingTextView.this.setText(String.valueOf(nowNum));
					}
					...
					if (nowNum < finalNum) {
						...
						//已经达到目标的数字，显示最终的内容
						if (useCommaFormat) {
							NumberRollingTextView.this.setText(StringUtil.addComma(String.valueOf(finalNum), false));
						} else {
							NumberRollingTextView.this.setText(String.valueOf(finalNum));
						}
					}
					break;
			}
