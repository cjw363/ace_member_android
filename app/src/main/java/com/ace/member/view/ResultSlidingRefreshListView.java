...
import com.og.component.RListViewHeader;

public class ResultSlidingRefreshListView extends ListView implements AbsListView.OnScrollListener {
	...
	//footer view
	private ResultListViewFooter mFooterView;
	...
	public ResultSlidingRefreshListView(Context context) {
		super(context);
		initWithContext(context);
	}

	public ResultSlidingRefreshListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initWithContext(context);
	}

	public ResultSlidingRefreshListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initWithContext(Context context) {
		...
		mHeaderViewContent = (RelativeLayout) mHeaderView.findViewById(com.og.R.id.lv_header_content);
		mHeaderTimeView = (TextView) mHeaderView.findViewById(com.og.R.id.lv_header_time);
		addHeaderView(mHeaderView);

		//init footer view
		mFooterView = new ResultListViewFooter(context);
		//init header height
	...
	public void setAdapter(ListAdapter adapter) {
		//make sure ResultListViewFooter is the last footer view,and only add once.
	...
	public void setPullLoadEnable(boolean enable) {
		...
		if (!mEnablePullLoad) {
			...
			mPullLoading = false;
			mFooterView.show();
			mFooterView.setState(ResultListViewFooter.STATE_NORMAL);
			//make sure "pull up" don't show a line in bottom when listview with one page
	...
	private void updateFooterHeight(float delta) {
				...
				int height=0;
				if(count<3){
					height = mFooterView.getBottomMargin() + (int) delta*10;
				}else{
		 height = mFooterView.getBottomMargin() + (int) delta;
				...
				if (mEnablePullLoad && !mPullLoading) {
					if (height > PULL_LOAD_MORE_DELTA) { // height enough to invoke loadmore.
						mFooterView.setState(ResultListViewFooter.STATE_READY);
					} else {
						mFooterView.setState(ResultListViewFooter.STATE_NORMAL);
					}
				}
	...
	private void startLoadMore() {
		mPullLoading = true;
		mFooterView.setState(ResultListViewFooter.STATE_LOADING);
...
//				final float deltaYY = ev.getRawY() - mLastY;
//				mLastY = ev.getRawY();
//				if (getLastVisiblePosition() == mTotalItemCount - 1 && (mFooterView.getBottomMargin() > 0 || deltaYY < 0)) {
//					// last item, already pulled up or want to pull up.
//					mFooterView.setState(ResultListViewFooter.STATE_LOADING);
//				}
//				break;
