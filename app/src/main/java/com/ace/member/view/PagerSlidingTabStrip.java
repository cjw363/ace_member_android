...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private LinearLayout.LayoutParams mSpecialTabLayoutParams;
	...
	private ColorStateList mSelectedTabTextColor = null;
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		mProportion = a.getInt(R.styleable.PagerSlidingTabStrip_pstsProportion, mProportion);
		...
		mSelectedTabTextColor = a.hasValue(R.styleable.PagerSlidingTabStrip_pstsSelectedTabTextColor) ? a
				.getColorStateList(R.styleable.PagerSlidingTabStrip_pstsSelectedTabTextColor) : null;
		...
		if (mTabTextColor == null) {
			mTabTextColor = createColorStateList(textPrimaryColor, textPrimaryColor, Color.argb(tabTextAlpha, Color
					.red(textPrimaryColor), Color.green(textPrimaryColor), Color.blue(textPrimaryColor)));
		}
		...
		if (mProportion > 0) {
			mSpecialTabLayoutParams = new LinearLayout.LayoutParams(0, LayoutParams.MATCH_PARENT, 1.5f);
		}
	}
	...
	public void notifyDataSetChanged() {
		...
		if (mTabBackgroundResId > 0) mTabsContainer.setBackgroundResource(mTabBackgroundResId);
		...
		for (int i = 0; i < mTabCount; i++) {
			if (isCustomTabs) {
				...
				tabView = LayoutInflater.from(getContext()).inflate(R.layout.view_psts_tab, this, false);
			}
	...
	private void addTab(final int position, CharSequence title, View tabView) {
		...
		if (mProportion - 1 == position) {
			mTabsContainer.addView(tabView, position, mSpecialTabLayoutParams);
		} else {
	...
	private void select(View tab) {
		if (tab != null) {
			...
			if (tab_title != null) {
				...
				tab_title.setTextColor(mSelectedTabTextColor);
			}
