...
public class ToPartnerActivity extends BaseCountryCodeActivity implements ToPartnerContract.View{


	@Inject
	ToPartnerPresenter mToPartnerPresenter;
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		DaggerToPartnerComponent.builder().toPartnerPresenterModule(new ToPartnerPresenterModule(this, this)).build().inject(this);
	...
	protected int getContentViewID() {
		return R.layout.activity_to_partner;
	}

	protected void initActivity(){
		setCurrencyList();
		ToolBarConfig.builder(this,null).setTvTitleRes(R.string.to_partner).setIvMenuRes(R.drawable.ic_history).setEnableMenu(true).build();
		...
		mToPartnerPresenter.start(true);
		mToPartnerPresenter.getBalance();
		try {
	...
	private void setListener() {
		SimpleTextWatcher.MoneyTextWatcher mMoneyTextWatcher = new SimpleTextWatcher.MoneyTextWatcher(mEtAmount) {
			...
			public void afterTextChanged(Editable editable) {
				mToPartnerPresenter.updateFeeInfo();
			}
		};
		...
		try {
			switch (view.getId()) {
				case R.id.ll_toolbar_menu:
					...
					it.putExtra("source", SOURCE_3_PARTNER);
					startActivity(it);
					break;
				...
				case R.id.btn_submit:
					mToPartnerPresenter.submit(mCurrency);
					break;
			}
		}catch (Exception e){
			FileUtils.addErrorLog(e);
	...
	public void showSuccess(String time) {
		...
		it.putExtra("from","from_to_partner");
		startActivity(it);
		clearInfo();
	}
