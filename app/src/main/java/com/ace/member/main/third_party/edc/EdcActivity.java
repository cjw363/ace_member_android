...
import com.ace.member.main.third_party.edc.history.EdcHistoryActivity;
import com.ace.member.main.third_party.edc.recent_contact.EdcRecentContactFragment;
...
public class EdcActivity extends BaseActivity implements EdcContract.View {

	@Inject
	EdcPresenter mPresenter;
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		DaggerEdcComponent.builder()
			.edcPresenterModule(new EdcPresenterModule(this, this))
			.build()
			.inject(this);
		init();
	}
	...
	private void init() {
		ToolBarConfig.builder(this, null)
			...
			.setTvTitleRes(R.string.electricity)
			.setMenuListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					Intent intent = new Intent(EdcActivity.this, EdcHistoryActivity.class);
	...
	public void onViewClicked(View view) {
		switch (view.getId()) {
			case R.id.rl_type:
				showSelectTypeDlg(mPresenter.getEdcWsaTypeArr());
				break;
			...
			case R.id.btn_submit:
				mPresenter.submitEdcWsa(mCurrentType, mEtNumber.getText()
					.toString()
					.trim(), mEtPhone.getText().toString().trim(), mEtAmount.getText()
					.toString()
					.trim(), mTvFee.getText().toString().trim(), mEtRemark.getText()
					.toString()
					.trim(), mList);
				break;
		}
	...
	private void showSelectRecentContact() {
		EdcRecentContactFragment fragment = new EdcRecentContactFragment();
		fragment.show(getSupportFragmentManager(), "EdcRecentContactFragment");
	}
	...
	private void toImageDetail(int current, View view) {
		Intent intent = new Intent(EdcActivity.this, ImageDetailActivity.class);
	...
	public void toPaymentHistoryDetailActivity(int billId) {
		Intent intent = new Intent(this, EdcHistoryDetailActivity.class);
	...
	@Subscribe
	public void onSelectRecentContact(SelectEdcRecentEvent event) {
		if (event == null) return;
		EdcBill bill = event.getEdcWsaBill();
		if (bill == null) return;
