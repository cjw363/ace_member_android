...
public class WsaHistoryDetailActivity extends BaseActivity implements WsaHistoryDetailContract.View {

	@Inject
	WsaHistoryDetailPresenter mPresenter;
	@BindView(R.id.iv_wsa)
	AppCompatImageView mIvEdcWsa;
	...
	private int mId;
	private WsaBill mBill;


	@Override
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		DaggerWsaHistoryDetailComponent.builder()
			.wsaHistoryDetailPresenterModule(new WsaHistoryDetailPresenterModule(this, this))
			.build()
			.inject(this);
	...
	protected int getContentViewID() {
		return R.layout.activity_wsa_history_detail;
	}
	...
	@Override
	public void setBill(WsaBill bill) {
		mBill = bill;
		mTvType.setText(AppUtils.getEdcWsaTypeArr(AppGlobal.PAYMENT_TYPE_2_WSA).get(bill.getType()));
		...
		mTvNumberTitle.setText(Utils.getString(bill.getType() == 1 ? R.string.bill_no : R.string.client_id));
	...
	public void toImageDetail() {
		...
		ActivityOptionsCompat options = ActivityOptionsCompat.makeScaleUpAnimation(mIvImage, 0, 0, mIvImage
			.getWidth(), mIvImage.getHeight());
