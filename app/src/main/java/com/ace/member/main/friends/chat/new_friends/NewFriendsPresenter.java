...
public class NewFriendsPresenter extends BasePresenter implements NewFriendsContract.NewFriendsPresenter {
	private static final int TYPE_NEW_FRIENDS_1_TITLE = 1;
	private final NewFriendsContract.NewFriendsView mView;

	@Inject
	public NewFriendsPresenter(Context context, NewFriendsContract.NewFriendsView mView) {
		super(context);
		this.mView = mView;
	}
	...
	public void getMayKnowFriends(List<ContactInfo> contactInfoList) {
		if (contactInfoList == null || contactInfoList.size() == 0) {//没有数据时添加标题
			mView.showMayKnowFriends(addTitle(new ArrayList<NewFriendsInfo>(), Utils.getString(R.string.people_you_may_know)));
			return;
		}
	...
	private List<NewFriendsInfo> addTitle(List<NewFriendsInfo> newFriendsInfoList, String title) {
		if (newFriendsInfoList != null) {//手动添加标题
