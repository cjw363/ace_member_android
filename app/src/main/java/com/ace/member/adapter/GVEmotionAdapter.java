...
public class GVEmotionAdapter extends BaseAdapter {

	private Context context;
	private List<String> emotionNames;
	private int itemWidth;

	public GVEmotionAdapter(Context context, List<String> emotionNames, int itemWidth) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ImageView iv_emotion = null;
		try {
			iv_emotion = new ImageView(context);
			// 设置内边距
			...
			if (position == getCount() - 1) {
				iv_emotion.setImageResource(R.drawable.ic_emotion_delete);
			} else {
