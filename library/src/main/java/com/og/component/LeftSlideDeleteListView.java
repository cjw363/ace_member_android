...
public class LeftSlideDeleteListView extends ListView {
	...
	private OnListViewItemDeleteClickListener deleteListener;
	/**
	 * 动画持续时间（毫秒）
	 */
...
//		});
	...
	public void setOnListViewItemDeleteClickListener(OnListViewItemDeleteClickListener listener) {
	...
	private void deleteClick(View v) {
		...
		for (int i = 0; i <= last - first; i++) {
			...
			if (positionTag == v.getTag()) {//v就是触发点击事件的按钮
				...
				if (sc.getChildAt(1).getTranslationX() != 0) {
					...
					if (itemClickListener != null) {
						itemClickListener.onItemClick(null, v, i + first, 0);
					}
				}
				break;
			}
   ...
   */
	...
	private class SlideContainer extends FrameLayout {
		...
		public SlideContainer(Context context, View topChild) {
			...
			delete.setText(getContext().getResources().getString(R.string.delete));
			...
			switch (ev.getAction()) {
				case MotionEvent.ACTION_MOVE:
					...
					if (dx > dis && dx > dy) {
						...
						} else getChildAt(1).setTranslationX(0);
	...
	private class ProxyAdapter implements ListAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {//null说明之前没缓存过（第一屏显示的View都是调用该方法得到的）
				SlideContainer sc = null;//为用户布局包裹支持左划的控件
				try {
					View v = targetAdapter.getView(position, null, parent);//调用用户adapter的getView方法得到用户布局
				...
				try {
					...
					} else sc = (SlideContainer) convertView;
