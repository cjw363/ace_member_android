...
public class MonthView extends View {
	...
	public MonthView(Context context, String date) {
		super(context);
	...
	private boolean mTouchEventTemp = true;
				...
				if (mTouchEventTemp) {
					if (mSlideMode == SlideMode.VER) {
						if (Math.abs(lastPointY - event.getY()) > 25) {
							if (lastPointY < event.getY() && centerYear > 2016) {
								if (Math.abs(lastPointY - event.getY()) >= criticalHeight) {
									indexYear--;
									centerYear = centerYear - 1;
								}
							} else if (lastPointY > event.getY() && centerYear < nowYear) {
								if (Math.abs(lastPointY - event.getY()) >= criticalHeight) {
									indexYear++;
									centerYear = centerYear + 1;
								}
						...
						if (Math.abs(lastPointX - event.getX()) > 25) {
							if (lastPointX > event.getX() && Math.abs(lastPointX - event.getX()) >= criticalWidth && (centerYear < nowYear || (centerYear == nowYear && centerMonth < nowMonth))) {
								indexMonth++;
							...
							} else if (lastPointX < event.getX() && Math.abs(lastPointX - event.getX()) >= criticalWidth &&(centerYear > 2016 || (centerYear == 2016 && centerMonth > 1))) {
								indexMonth--;
	...
	private void draw(Canvas canvas, Rect rect, DPInfo info) {
		...
		drawGregorian(canvas, rect, info.strG, info.isWeekend, info.isExceedToday);
	...
	private void drawBG(Canvas canvas, Rect rect, DPInfo info) {
		...
		if (!TextUtils.isEmpty(info.strG)) {
			day = decimalFormat.format(Integer.valueOf(info.strG));
		}
		if ((centerYear + "-" + decimalFormat.format(centerMonth) + "-" + day).equals(selectedDate)) {
			drawBGToday(canvas, rect);
		}
		//		else if (info.isToday && isTodayDisplay) {
		//			drawBGToday(canvas, rect);
		//		}
		else {
	...
	private void drawGregorian(Canvas canvas, Rect rect, String str, boolean isWeekend, boolean isExceedToday) {
		mPaint.setTextSize(sizeTextGregorian);
		if (isWeekend) {
		...
		if (isExceedToday) {
			mPaint.setColor(mTManager.colorExceedToday());
		}
		float y = rect.centerY() + offsetYFestival1 / 2F;
	...
	void setDate(int year, int month) {
		...
		nowYear = Integer.valueOf(format.format(date));
		nowMonth = Integer.valueOf(format1.format(date));
		indexYear = 0;
		indexMonth = 0;
		buildRegion();
		computeDate();
	...
	private void computeDate() {
		...
		isMaxYear = (bottomYear > nowYear || rightYear > nowYear);
